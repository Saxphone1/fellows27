...
object SparkJoin {
  def main(args: Array[String]): Unit = {
    ...
    val lines: Dataset[String] = spark.createDataset(List("1,laozhoa,china", "2,laoduan,usa"))
    //对数据进行整理
    ...
    //对数据进行整理
    val ndataset: Dataset[(String, String)] = nations.map(x => {
      val fields = x.split(",")
      val ename = fields(0)
      val cname = fields(1)
      (ename, cname)
...
//    val r: DataFrame = spark.sql("SELECT name, cname FROM v_users JOIN v_nations ON nation = ename")
    ...
    val r = df1.join(df2, $"nation" === $"ename").select($"name",$"cname")
